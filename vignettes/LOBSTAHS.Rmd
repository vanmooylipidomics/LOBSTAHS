...
##### The two primary `DB_gen_compound_type`s in the `componentCompTable`

The other two compound types ("DB_acyl_iteration" and "DB_unique_species) are used to define the way `generateLOBdbase` creates database entries for different lipids and lipid classes. `generateLOBdbase` essentially can creates two kinds of database entries:
