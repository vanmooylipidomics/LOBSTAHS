...
The other two compound types in the `componentCompTable` ("DB_acyl_iteration" and "DB_unique_species) are used to define the way `generateLOBdbase` creates database entries for different lipids and lipid classes. `generateLOBdbase` essentially can creates two kinds of database entries:

1. When "DB_unique_species" is specified for the `DB_gen_compound_type`, `generateLOBdbase()` will create database entries only for adduct ions of the single compound specified. (This is the simpler of the two cases.) This type should be used for pigments and other lipids that do not have acyl groups, or in cases when the user does not wish to consider any possible variation in acyl properties. *In the case of this `DB_gen_compound_type`, the exact mass of the complete (neutral) molecule is specified in the component definitions table.* The photosynthetic pigments currently in the database provide an excellent template for creation of new compounds of the "DB_unique_species" type.

1. When "DB_acyl_iteration" is specified for the `DB_gen_compound_type`, `generateLOBdbase` will create database entries for adduct ions of multiple molecular species within a lipid class based on the ranges of acyl properties and oxidation states specified in two other user-editable tables, `acylRanges` and `oxyRanges`:
...
<img src="images/Fig2.jpg" width="695" id="Intact PC 18:1, 18:2">

This intact molecule has the chemical formula C<sub>44</sub>H<sub>84</sub>NO<sub>8</sub>P an exact mass of **785.59346** amu. The C-1 carbon atom in both fatty acids is shown explicitly for ease of illustration. Using the <a href="#Base-fragment-def">rules above</a>, we define the base fragment for PC (and all other IP-DAG) as the polar headgroup, glycerol backbone, and both the carboxylic oxygen atoms (in red):

<img src="images/Fig3.jpg" width="695" id="PC base fragment in red">

In this case, the base fragment (shown in red) has the chemical formula C<sub>8</sub>H<sub>18</sub>NO<sub>8</sub>P a mass of **287.07700** amu. One "easy" way to determine the formula (and therefore, the exact mass) of the base fragment is to use the fragmentation feature in an application such as ChemDraw. First, we fragment the bond(s) between the C-1 and C-2 carbon atom(s) in each acyl chain on which iteration is to be performed:

<img src="images/Fig4.jpg" width="695" id="Fragmentation illustration, part 1">

We then subtract the appropriate number of carbon atoms and their masses:

<img src="images/Fig5.jpg" width="695" id="Fragmentation illustration, part 2">
